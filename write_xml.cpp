...
void WriteXML::WriteCompound(MV_FILE* compound_vox, string vox_file, XMLElement* compound_xml, Shape* shape, int i, int j, int k) {
	...
	// TODO: FIGURE OUT AN ALGORITM FOR THIS
	// The "code" below is a hell of a hack, and doesn't work well at all.
	// <this comment was generated by Github Copilot>

	int parts_x = (sizex + 256 - 1) / 256;
	int shape_offset = -(sizex % 256) / 20;
	float pos_x = 0;
	if (i == parts_x - 1)
		pos_x = i * 12.8;
	else if (parts_x == 2)
	...
	//int parts_y = (sizey + 256 - 1) / 256;
	//float pos_y = parts_y == 1 ? 0 : -25.6 * (j-1) - (sizey % 256) / 20;
		...
		if (joint->type != _Rope) {
			...
			if (shape_parent != NULL) {
				if (entity_mapping.find(shape_handle) != entity_mapping.end()) {
					...
					Vector relative_pos = { shape_tr.pos.x, joint->shape_positions[0][1], joint->shape_positions[0][2] };
					Transform joint_tr = TransformToLocalTransform(shape_tr, Transform(relative_pos, Quat()));

					/*Entity* parent = entity->parent;
					if (parent != NULL && parent->kind_byte == KindBody) {
						Body* body = (Body*)parent->kind;
						Transform parent_tr = body->transform;
					}*/
